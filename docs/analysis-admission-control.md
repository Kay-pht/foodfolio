...
日次上限の1日は **00:00〜23:59 JST** とする。DBの日時自体は従来どおりUTCで保存する。
...
日次上限は解析成功数ではなく **受付数** を数える。enqueue失敗や解析失敗でも `AnalysisAdmission` 自体は残すため、失敗を利用して日次上限を回避できない。
