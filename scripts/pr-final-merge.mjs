...
function readPr() {
  return JSON.parse(run("gh", ["pr", "view", "--json", "number,headRefOid,state,isDraft"]));
}
...
if (!existsSync(proofPath)) {
  throw new Error(`Review proof not found: ${proofPath}. A Reviewer must run npm run review:lgtm first.`);
}

const proof = JSON.parse(readFileSync(proofPath, "utf8"));
if (proof.version !== 1 || proof.verdict !== "LGTM" || !proof.pr || !proof.reviewed_sha) {
...
if (pr.state !== "OPEN" || pr.isDraft) {
  throw new Error(`PR #${pr.number} must be open and ready for review before merge.`);
}
if (pr.number !== proof.pr) {
  throw new Error(`Review proof is for PR #${proof.pr}, but current branch belongs to PR #${pr.number}.`);
}
...
run("gh", ["pr", "checks", String(pr.number), "--watch", "--fail-fast"], { inherit: true });
...
console.log("All final checks passed. Executing human-requested SHA-bound merge...");
run(
  "gh",
  [
    "pr",
