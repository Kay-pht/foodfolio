...
- [ ] レートリミットを設ける。現状、ユーザーはai解析を使い放題になっている。それでは費用も無限に掛かることになるので、ユーザーあたりの解析回数をリミットする。無料だし1日あたり10回までとかが妥当か？
- [ ] ログインより先にai解析に同意するかの方を先にするのはどうか？これがなければこのアプリは使えないも同然なので。これに同意してからログインに進める仕様でどうか？ログイン＝このアプリを利用って感じで明快だし
