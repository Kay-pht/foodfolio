...
export class RecipeAnalysisService {
  ...
  async process(
    ...
    const recipe = await this.deps.prisma.recipe.findUnique({ where: { id: recipeId } });
    if (!recipe || recipe.analysisStatus === "completed" || recipe.analysisStatus === "failed")
      return { retry: false };

    const runId = randomUUID();
    const claimed = await this.claim(recipeId, runId, attempt);
    if (!claimed) {
      log({ recipeId, analysisAttempt: attempt }, "recipe analysis is already processing");
      return { retry: true };
    }
    ...
    try {
      const source = await this.deps.sourceExtractor.extract(new URL(recipe.originalUrl));
      ...
      const committed = await this.deps.prisma.$transaction(async (tx) => {
        const owned = await tx.recipe.findFirst({
          where: { id: recipeId, analysisStatus: "processing", processingRunId: runId },
          select: { id: true },
        });
        ...
        if (result.recipe.steps.length)
          await tx.recipeStep.createMany({
            data: result.recipe.steps.map((text, sortOrder) => ({ recipeId, text, sortOrder })),
          });
      ...
      const analysisError =
        error instanceof AnalysisError
          ? error
          : new AnalysisError("INTERNAL_ANALYSIS_ERROR", true, "Unexpected analysis error");
      const final = !analysisError.retryable || attempt >= this.deps.maxAttempts;
      ...
      if (final) await this.notify(recipe.userId, recipeId, title, "failed", log);
      return { retry: !final };
    }
  }

  private async claim(recipeId: string, runId: string, attempt: number): Promise<boolean> {
      ...
      const claimed = await tx.recipe.updateMany({
        where: {
          ...
          OR: [
            ...
            { analysisStatus: "processing", processingLeaseExpiresAt: { lte: now } },
            ...(attempt > 1
              ? [{ analysisStatus: "processing" as const, processingLeaseExpiresAt: null }]
              : []),
          ],
        },
        data: {
  ...
  private async resultAfterLostOwnership(recipeId: string): Promise<{ retry: boolean }> {
  ...
  private async notify(
    ...
    const setting = await this.deps.prisma.userSetting.findUnique({ where: { userId } });
    if (!setting?.recipeAnalysisNotificationEnabled) return;
    const tokenRows = await this.deps.prisma.deviceToken.findMany({ where: { userId } });
    try {
      const tokens = tokenRows.map(({ fcmToken }) => fcmToken);
      const invalid =
        result === "completed"
          ? await this.deps.notifications.sendRecipeAnalysisCompleted(tokens, recipeId, title)
          : await this.deps.notifications.sendRecipeAnalysisFailed(tokens, recipeId, title);
      if (invalid.length)
        await this.deps.prisma.deviceToken.deleteMany({ where: { fcmToken: { in: invalid } } });
    } catch (error) {
      log(
